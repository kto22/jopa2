...
async def bandit(message):
    ...
    if message.from_user.id == 598970197:
        await message.answer('а ты каримбумчик')

    await message.answer('верно ли прдсказание?', reply_markup=markup)
...
@dp.message_handler(lambda message: message.text == '122334455667')
